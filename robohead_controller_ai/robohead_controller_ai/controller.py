...
class RoboheadControllerAI(Node):
    def __init__(self):
        super().__init__('robohead_controller_ai')
        ...
        self.llm_url = self.declare_parameter(
            'llm.url', 'http://robohead0llm.local:11434/api/generate').value
        self.llm_model = self.declare_parameter(
            'llm.model', 'qwen3:4b-instruct').value
        self.llm_timeout = self.declare_parameter(
            'llm.timeout', 120).value
        self.llm_temperature = self.declare_parameter(
            'llm.temperature', 0.3).value
        self.llm_num_predict = self.declare_parameter(
            'llm.num_predict', 1024).value

        system_prompt = self.declare_parameter(
            'llm.system_prompt', '').value
        function_description = self.declare_parameter('llm.function_description', '').value
        self.llm_system_prompt = system_prompt + function_description

        # format_schema: строка JSON → dict
        format_schema_str = self.declare_parameter(
            'llm.format_schema', '').value
        self.llm_format_schema = None
        if format_schema_str:
            try:
                self.llm_format_schema = json.loads(format_schema_str)
            except json.JSONDecodeError as e:
                self.get_logger().error(f'robohead_llm_controller: Invalid format_schema JSON: {e}')
        system_prompt += "\nФормат ответа:\n" + format_schema_str 
