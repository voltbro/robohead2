    ...
    def queue_tick(self):
        ...
        if self.state == 'wait_wake_phrase':
            if len(self.controller.queue_wake_phrases) > 0:
                ...
                self.controller.speech_recognizer_asr.set_mode(2)
            ...
            if len(self.controller.queue_frees) > 0:
                ...
                command = self.controller.queue_frees.pop(0)
                # self.controller.get_logger().info("comander 7")

                if command != self.controller.speech_recognizer_asr.timeout_text:
                    self.controller.action_manager.execute_action("std_loading", None, True)
                    self.controller.get_logger().info(f"user_promt: {command}")
                
                    self.controller.action_manager.execute_action_ai(command, 'std_wait', True)
                else:
                    self.controller.action_manager.execute_action("std_wait", None, True)
                
                self.controller.queue_frees.clear()
