...
from gyrobro_controller.main import RoboheadController
...
def run(controller, action_name: str, cancel_event_: threading.Event):
    """
    Откалибровать цвет шарика, затем отслеживать его и двигать шеей.
    Прерывается по cancel_event.
    """
    global robohead, is_run, cancel_event
    robohead = controller
    ...
    robohead.media_driver.play_media(cancel_event,
    video_path="/home/pi/robohead_ws/src/robohead2/gyrobro_controller/gyrobro_controller/actions/std_ball_tracker/video.mp4",
    audio_path="/home/pi/robohead_ws/src/robohead2/gyrobro_controller/gyrobro_controller/actions/std_ball_tracker/calibrate_voice.mp3",
    ...
    robohead.media_driver.play_media(cancel_event,
    video_path="/home/pi/robohead_ws/src/robohead2/gyrobro_controller/gyrobro_controller/actions/std_ball_tracker/video.mp4",
    audio_path="/home/pi/robohead_ws/src/robohead2/gyrobro_controller/gyrobro_controller/actions/std_ball_tracker/finish_voice.mp3",
    loop=False, block=True
    )
...
def move_ears(left_angle, right_angle, duration):
    ...
    robohead.ears_driver.set_angle(cancel_event, left=left_angle, right=right_angle, duration=duration, block=True)
...
def neck_mover(cancel_event: threading.Event):
    ...
    prev_x=None
    prev_y=None
    start_timer = robohead.get_clock().now().nanoseconds

    while is_run and not cancel_event.is_set():
        x, y = ball_xy

        if (robohead.get_clock().now().nanoseconds-start_timer) > 1:
            msg_cmd_vel = Twist()
            robohead.gyrobro.cmd_vel(msg_cmd_vel)
            start_timer = robohead.get_clock().now().nanoseconds
        ...
        if (delta_x**2) > threshold_zone**2:
            velocity = float(min(max(0.2*abs(delta_x)/(resized_camera_resolution[0]/4), 0.1), 0.4))
            msg_cmd_vel.angular.z = -velocity*sign(delta_x)
        else:
...
def ball_tracker(cancel_event: threading.Event):
    ...
    while is_run and not cancel_event.is_set():
        current_img = robohead.usb_cam.image_raw
        ...
        try:
            ...
            robohead.media_driver.stream_publish(msg)
        except Exception as e:
