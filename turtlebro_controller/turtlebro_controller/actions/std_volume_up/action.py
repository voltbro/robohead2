# std_volume_up
# действие, выполняющееся при команде "Громче"
...
def run(
    ...
    elif controller.media_driver.is_idle_audio(cancel_event=cancel_event):
        controller.media_driver.play_audio(
            cancel_event=cancel_event,
            audio_path=os.path.join(action_dir, "max_vol.mp3"),
            loop=False,
            block=False,
        )
