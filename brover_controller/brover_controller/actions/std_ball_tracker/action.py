...
class BallTrackerApp:
    ...
    def neck_thread(self):
        ...
        while self.is_run and not self.cancel_event.is_set():
            ...
            if (x < (center_x - THRESHOLD_ZONE)) or (x > (center_x + THRESHOLD_ZONE)):
                d_ = (x - center_x) / center_x
                self.controller.brover_driver.velocity_publish(linear_x=0.0, angular_z=d_)
            else:
                self.controller.brover_driver.velocity_publish(linear_x=0.0, angular_z=0.0)
